            ...
            if all(x in current_id for x in network_station):
                ...
                for current_latency in current_NSC['intervals']:
                    combined_latency_data_for_all_days, \
                        current_day_latency_data = get_latency_value_for_current_timestamp(
                            current_latency=current_latency,
                            combined_latency_data_for_all_days=combined_latency_data_for_all_days,
...
def get_latency_value_for_current_timestamp(current_latency: dict,
                                            ...
                                            current_network: str,
                                            current_station: str,
                                            current_channel: str) -> Tuple[dict, dict]:
    if current_latency["retx"]["allPackets"] == 1:
        if current_latency["latency"]["maximum"] != -1:
            append_to_latency_objects(current_latency=current_latency["latency"]["maximum"],
                                      combined_latency_data_for_all_days=combined_latency_data_for_all_days,
                                      current_day_latency_data=current_day_latency_data,
                                      current_network=current_network,
                                      current_station=current_station,
        ...
        if current_latency["latency"]["minium"] != -1:
            append_to_latency_objects(current_latency=current_latency["latency"]["minium"],
                                      combined_latency_data_for_all_days=combined_latency_data_for_all_days,
                                      current_day_latency_data=current_day_latency_data,
                                      current_network=current_network,
                                      current_station=current_station,
        ...
        if current_latency["latency"]["average"] != -1:
            ...
            append_to_latency_objects(current_latency=calculated_unkown_latency_value,
                                      combined_latency_data_for_all_days=combined_latency_data_for_all_days,
                                      current_day_latency_data=current_day_latency_data,
                                      current_network=current_network,
                                      current_station=current_station,
        ...
        if current_latency["latency"]["average"] != -1:
            ...
            for iteration in range(number_of_times_to_append_to_latencies):
                append_to_latency_objects(current_latency=current_latency["latency"]["average"],
                                          combined_latency_data_for_all_days=combined_latency_data_for_all_days,
                                          current_day_latency_data=current_day_latency_data,
                                          current_network=current_network,
                                          current_station=current_station,
...
combined_latency_dataframe_for_all_days_dataframe, \
    array_of_daily_latency_dataframes = calculate_apollo_latencies(files=["tests/data/apollo/archive/latency/2022/03/28/QW.BCV13.2022.087.json",
                                                                          "tests/data/apollo/archive/latency/2022/03/29/QW.BCV13.2022.088.json"],
                                                                   network="QW",
                                                                   station="BCV13")
print("combined_latency_dataframe_for_all_days_dataframe",
      combined_latency_dataframe_for_all_days_dataframe)
print("array_of_daily_latency_dataframes", array_of_daily_latency_dataframes)
