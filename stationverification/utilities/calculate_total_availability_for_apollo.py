...
def calculate_total_availability(files: list):
    ...
    for file in files:
        ...
        for latency_value in latency_array:
            local_sum_of_percent_availability += \
                latency_value["percentAvailability"]
        total_sum_of_percent_availability +=\
            local_sum_of_percent_availability / \
            length_of_latency_array
    total_availability_percentage = math.floor(
        total_sum_of_percent_availability / length_of_latency_files * 10 ** 2)\
        / 10 ** 2
